...
assert_true(User::can($client, 'book_sessions'), 'client can book counselling sessions');
...
$bookingService = new SessionBookingService();
$discovery = $bookingService->discovery(['focus' => 'Anxiety']);
assert_true(count($discovery['profiles']) >= 1, 'booking discovery returns counsellor profiles');
assert_true(count($discovery['profiles'][0]['next_slots']) >= 1, 'booking discovery returns availability slots');
assert_true(str_contains($discovery['safety_note'], 'assistive only'), 'booking AI matching has safety note');
$slot = $discovery['profiles'][0]['next_slots'][0];
$hold = $bookingService->createHold((int) $client['id'], [
    'counsellor_profile_id' => $slot['counsellor_profile_id'],
    'starts_at' => $slot['starts_at'],
    'timezone' => 'Africa/Lagos',
]);
assert_true($hold['ok'] === true, 'client can hold an available booking slot');
$duplicateHold = $bookingService->createHold((int) $client['id'], [
    'counsellor_profile_id' => $slot['counsellor_profile_id'],
    'starts_at' => $slot['starts_at'],
    'timezone' => 'Africa/Lagos',
]);
assert_true($duplicateHold['ok'] === false, 'booking hold prevents double booking');
$confirmedBooking = $bookingService->confirm((int) $client['id'], (int) $hold['hold']['id'], ['accessibility' => ['captions', 'low-bandwidth']]);
assert_true($confirmedBooking['ok'] === true, 'client can confirm a held booking slot');
assert_true((int) $confirmedBooking['booking']['amount_minor'] > 0, 'booking stores payment amount in integer minor units');
$rescheduled = $bookingService->reschedule((int) $client['id'], (int) $confirmedBooking['booking']['id'], '2026-05-18 16:00');
assert_true($rescheduled['ok'] === true, 'client can reschedule a confirmed booking');
$cancelled = $bookingService->cancel((int) $client['id'], (int) $confirmedBooking['booking']['id'], 'Test cancellation.');
assert_true($cancelled['ok'] === true, 'client can cancel a booking calmly');
assert_true(count(Database::table("SELECT * FROM audit_logs WHERE action LIKE 'booking_%'")) >= 4, 'booking actions are audited');
assert_true(count(Database::table('SELECT * FROM booking_events')) >= 4, 'booking events are recorded');

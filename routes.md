...
- `GET /booking/data` — JSON counsellor discovery, filters, slots, and AI-match safety notes.
- `POST /booking/hold` — hold an available slot with double-booking prevention.
- `POST /booking/confirm` — confirm a held slot with payment/reminder readiness.
- `POST /booking/reschedule` — reschedule a confirmed session when a slot is available.
- `POST /booking/cancel` — cancel a confirmed session with calm audit logging.
